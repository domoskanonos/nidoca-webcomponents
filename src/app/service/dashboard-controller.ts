...
export class DashboardController {
  ...
  static getVertraegeVertragsendeErreichtUndNichtGekuendigt(): any[] {
    ...
    return alleVertraege.filter(
      (item: any) =>
        item.aktiv &&
        !item.gekuendigt &&
        item.vertragsende &&
        item.vertragsende.getTime() - item.kuendigungsfrist * 1000 * 60 * 60 * 24 < new Date().getTime()
    );
  }
  ...
  static alleKostenpflichtigeVertraegeChart(): ChartConfiguration | undefined {
    ...
    if (vertraege == undefined) return undefined;
