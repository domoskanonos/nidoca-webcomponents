   ...
   static styles = css`
      ...
      *::after {
      ...
      [anim="ripple"]:before {
         ...
         -webkit-transition: calc(var(--t, 0) * var(--ripple-duration, 600ms))
   ...
   render(): TemplateResult {
      return html`
         <div
            id="rippleContainer"
            anim="ripple"
            @click="${(event: MouseEvent | TouchEvent) => this.clicked(event)}"
         >
            <slot></slot>
         </div>
      `;
   ...
   private clicked(event: MouseEvent | TouchEvent | Touch) {
      if (this.rippleContainerElement != undefined) {
         ...
         if (event instanceof MouseEvent || event instanceof Touch) {
            this.rippleContainerElement.style.cssText = `--t: 1; --o: 0; --d: ${d}; --x:${
